	...
	const unsigned char *sk,unsigned char *c_bytes,unsigned char *f_bytes,unsigned char *t_bytes)
{
  ...
  fpr c_read[512],f_read[512],t_read[512];
#if KG_EXPAND
	Zf(sign_tree)(sig, &sc, expanded_key, hm, LOGN, tmp.b);
#else
	Zf(sign_dyn)(sig, &sc, f, g, F, G, hm, LOGN, tmp.b,c_read,f_read,t_read);
#endif
    memcpy(c_bytes, c_read, 512 * 8);
    memcpy(f_bytes, f_read, 512 * 8);
    memcpy(t_bytes, t_read, 512 * 8);
	...
	sm[1] = (unsigned char)sig_len;  //sm[0][1]signature length
